...
async function getProducts(filters: ProductFilters): Promise<Product[]> {
  ...
  if (filters.search) {
    conditions.push(
      `(name ILIKE $${idx} OR description ILIKE $${idx} OR cheese_type ILIKE $${idx})`,
    );
  ...
  const orderBy =
    sortMap[filters.sort || "popular"] || "featured DESC, sort_order ASC";
...
// 1. Updated interface to use a Promise
interface ShopPageProps {
  searchParams: Promise<{ [key: string]: string | undefined }>;
}

export default async function ShopPage({ searchParams }: ShopPageProps) {
  // 2. Await the searchParams before use
  const sp = await searchParams;

  const filters: ProductFilters = {
    search: sp.search,
    cheese_type: sp.type,
    min_price: sp.min ? parseFloat(sp.min) : undefined,
    max_price: sp.max ? parseFloat(sp.max) : undefined,
    in_stock: sp.instock === "true" ? true : undefined,
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-10 md:py-14">
      {/* Page header */}
      <div className="mb-10">
        <p className="text-cheese-600 text-sm font-medium mb-2 uppercase tracking-wide">
          Assortiment
        </p>
        ...
        <div className="flex-1">
          {products.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 text-earth-400">
              ...
              <p className="text-sm">
                Probeer een andere zoekopdracht of verwijder filters
              </p>
            </div>
          )}
        </div>
