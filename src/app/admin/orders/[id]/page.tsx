...
async function getOrder(id: string): Promise<(Order & { items: OrderItem[] }) | null> {
  ...
  return {
    ...(o as Order),
    total_amount: parseFloat(o.total_amount as string),
    subtotal: parseFloat(o.subtotal as string),
    delivery_cost: parseFloat(o.delivery_cost as string),
    items: ((o.items as OrderItem[]) || []).filter((i: OrderItem) => i.id).map((i: OrderItem) => ({
      ...i,
      unit_price: parseFloat(i.unit_price as unknown as string),
...
export default async function AdminOrderDetailPage({ params }: Props) {
  ...
  return (
    <div>
      <Link href="/admin/orders" className="inline-flex items-center gap-1 text-sm text-earth-500 hover:text-cheese-600 mb-6 transition-colors">
        <ChevronLeft size={16} /> Terug naar bestellingen
      </Link>

      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="font-serif text-3xl text-earth-900">Bestelling #{order.order_number}</h1>
          <p className="text-earth-500 mt-1">{formatDateTime(order.created_at)}</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="card overflow-hidden">
              ...
              {order.items.map(item => (
                <div key={item.id} className="flex items-center gap-4 px-5 py-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-earth-900">{item.product_name}</p>
                    {item.weight_option && <p className="text-sm text-earth-400">{item.weight_option}</p>}
                    <p className="text-sm text-earth-500">{item.quantity}× {formatPrice(item.unit_price)}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="font-medium text-earth-900">{formatPrice(item.subtotal)}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="p-5 border-t border-earth-100 bg-earth-50 space-y-1 text-sm">
              ...
              <div className="flex justify-between font-semibold text-base text-earth-900 pt-1 border-t border-earth-200 mt-1">
                <span>Totaal</span>
                <span className="font-serif text-lg text-cheese-700">{formatPrice(order.total_amount)}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-5">
          ...
          <div className="card p-5">
            <h2 className="font-serif text-lg text-earth-900 mb-4">Klantgegevens</h2>
            <div className="space-y-3 text-sm">
              ...
              <a href={`mailto:${order.customer_email}`} className="flex items-center gap-2 text-earth-600 hover:text-cheese-600 transition-colors">
                <Mail size={14} className="shrink-0" />
                {order.customer_email}
              </a>
              {order.customer_phone && (
                <a href={`tel:${order.customer_phone}`} className="flex items-center gap-2 text-earth-600 hover:text-cheese-600 transition-colors">
          ...
          {/* Delivery address */}
          <div className="card p-5">
            <h2 className="font-serif text-lg text-earth-900 mb-3">Bezorgadres</h2>
          ...
          {order.notes && (
            <div className="card p-5">
              <h2 className="font-serif text-lg text-earth-900 mb-2">Opmerking klant</h2>
          ...
          <div className="bg-cheese-50 border border-cheese-200 rounded-xl p-4 text-sm text-cheese-800">
            ...
            <p className="text-cheese-700">Herinner de klant te betalen bij ontvangst van de bestelling.</p>
          </div>
        </div>
      </div>
    </div>
