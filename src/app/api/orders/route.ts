...
const OrderSchema = z.object({
  ...
  delivery_postal_code: z.string().regex(/^\d{4}\s?[A-Z]{2}$/i, 'Ongeldige postcode'),
...
export async function GET(req: NextRequest) {
  ...
  if (search) {
    conditions.push(`(o.customer_name ILIKE $${idx} OR o.customer_email ILIKE $${idx} OR o.order_number ILIKE $${idx})`)
    params.push(`%${search}%`)
    idx++
  }
  ...
  return NextResponse.json({
    ...
    page,
    pageSize,
    totalPages: Math.ceil(parseInt((countResult[0] as { count: string }).count) / pageSize),
  })
}
